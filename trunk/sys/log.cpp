...
void CLogger::logMessage(int level,const char *file, int line, 
	                     const char *function, pthread_t tid, const char *fmt, ...) {
   	if (level>_level) {
   		return;
   	}
	...
	if (WarningBuffer::is_warn_log_on() && data_size > 0) {
		if (level == SYS_LOG_LEVEL_WARN){ // WARN only
...
void CLogger::checkFile() {
	...
	if ((err == -1 && errno == ENOENT)
			|| (err == 0 && (stFile.st_dev != stFd.st_dev 
			|| stFile.st_ino != stFd.st_ino))) {
		int fd = open(_name, O_RDWR | O_CREAT | O_APPEND | O_LARGEFILE, LOG_FILE_MODE);
		if (!_flag) {
			dup2(fd, _fd);
			dup2(fd, 1); 
...
void CLogger::setMaxFileSize( int64_t maxFileSize) { // 1GB   
