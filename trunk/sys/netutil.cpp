 ...
 * 得到本机ip
 */
uint32_t netUtil::getLocalAddr(const char *dev_name) {
    int             fd, intrface;
    struct ifreq    buf[16];
    struct ifconf   ifc;
 ...
 */
bool netUtil::isLocalAddr(uint32_t ip, bool loopSkip) {
    int             fd, intrface;
    struct ifreq    buf[16];
    struct ifconf   ifc;
 ...
 */
uint32_t netUtil::getAddr(const char *ip) {
    if (ip == NULL) {
        return 0;
    }
 ...
 * 把uint64转成字符串
 */
string netUtil::addrToString(uint64_t ipport) {
    char str[32];
 ...
 * 把ip,port转成uint64_t
 */
uint64_t netUtil::strToAddr(const char *ip, int port) {
    uint32_t nip = 0;
 ...
 * 把ip,port转成uint64_t
 */
uint64_t netUtil::ipToAddr(uint32_t ip, int port) {
    uint64_t ipport = port;
    ipport <<= 32;
    ipport |= ip;
